...
public class MyDatabase extends SQLiteOpenHelper {
    ...
    public List<Job> getAllJob() {
        ...
        while (cursor.moveToNext()) {
            ...
            Job job = new Job( name, note, NgayBatDau, NgayketThuc, trangThai);
            Jobs.add(job);
        }
    ...
    {
        SQLiteDatabase database=getWritableDatabase();
        List<Job> nhanVienList=getAllJob();
        ContentValues values=new ContentValues();
        // values.put("Id",job.id);
        values.put("Name",job.Name);
        values.put("Note",job.Note);
        ...
        long kq= database.update("Job",values,"Name=?",new String[]{job.getName()});
        getAllJob();
        return kq;
    }
    ...
    {
        SQLiteDatabase database=getWritableDatabase();
        long kq= database.delete("Job","Name=?",new String[]{job.getName()+""});
        return  kq;
    }
}
